...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        config_args['vocab_size'] = 50257  # Always 50257 for GPT-2 model
        config_args['block_size'] = 1024  # Always 1024 for GPT-2 model

        # Initialize model from scratch with minGPT-like architecture
        ...
        for k in sd_keys_hf:
            if any(k.endswith(w) for w in transposed):
                # Transpose special Conv1D weights
...
if ddp:
    # use of DDP atm demands CUDA, we set the device appropriately accordingly to rank
    ...
    master_process = ddp_rank == 0 # process will do logging, checkpointing etc
else:
    # vanilla, non-DDP run
    ddp_rank = 0
    ddp_local_rank = 0
...
total_batch_size = 524388 # 2**19, ~0.5M, in number of tokens
...
use_compile = False # torch.compile interferes with HellaSwag evaluation and Generation -- fix?
if use_compile:
    model = torch.compile(model)
if ddp:
...
def get_lr(it):
    ...
    if it > max_steps:
        return min_lr
    # 3) in between, use cosine decay down to run min learning rate
...
# create log directory we will write checkpoints to and log to
log_dir = "log"
...
for step in range(max_steps):
    ...
    # disabled because torch.compile throws unsolvable error.
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
    optimizer.step()
    # Synchronize if using CUDA, omit for MPS (or CPU)
    if device == "cuda":
